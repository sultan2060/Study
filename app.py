...
try:
  ...
  if not live_df.empty:
    ...
    prev_p = (
        float(live_df["Close"].iloc[-2]) if len(live_df) > 1 else current_p
    )
  ...
  st.warning("تعذر تحديث البيانات اللحظية حالياً.")

st.markdown("---")

# رادار الأخبار الحالية مع تصحيح فتح الروابط في نافذة جديدة تماماً
if lang == "العربية":
  st.markdown(
...
# عرض سجل البحث والأداء التجريبي (مقارنة البرنامج بالسوق المباشر)
if lang == "العربية":
  st.markdown(
...
if os.path.exists(REGISTER_FILE):
  ...
  if total_records > 0:
    ...
    hit_rate = (
        (success_hits / total_records) * 100 if total_records > 0 else 0.0
    )
  ...
  st.info(
      "جاري تحميل سجل الرصد الأوتوماتيكي..."
      if lang == "العربية"
      else "Loading automated register..."
  )
